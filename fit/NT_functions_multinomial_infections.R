...
calc_likelihood_overall <- function (theta_season, init_state_season, season_data, log = FALSE)
{
  ...
  lik_table$Dual_modelcases_1 <- lik_table$Dual_incidence_1*theta_season[["Rdetect2"]]*theta_season[["RSV_mult"]]
  lik_table$Dual_modelcases_2 <- lik_table$Dual_incidence_2*theta_season[["Rdetect5"]]*theta_season[["RSV_mult"]]
...
# likelihood_dual_points<- function(data_point, model_point, theta, i, log = FALSE)
# {# calculate the likelihood of each point per age group using poisson distributions.
#   
#   l <- dnbinom(x = data_point[[i,"both_0"]],
#                mu = model_point[[i,"Dual_incidence_1"]]*theta[["Rdetect2"]]*theta[["RSV_mult"]] ,
#                size = theta[["overdispersion"]]  # Infinity is a poisson
#                , log=T)
#   
#   m <- dnbinom(x = data_point[[i,"both_1"]],
#                mu = model_point[[i,"Dual_incidence_2"]]*theta[["Rdetect5"]]*theta[["RSV_mult"]],
#                size = theta[["overdispersion"]] # Infinity is a poisson
#                , log=T)
#   
#   return(l + m)
...
# likelihood_rsv_points<- function(data_point, model_point, theta, i, log = FALSE)
# {# calculate the likelihood of each point per age group using poisson distributions.
#   
#   
#   l <- dnbinom(x = data_point[[i,"Hos_RSV_0"]],
#                mu = model_point[[i,"RSV_incidence_1"]]*theta[["Rdetect2"]]*theta[["RSV_mult"]] ,
#                size = theta[["overdispersion"]]  # Infinity is a poisson
#                , log=T)
#   
#   m <- dnbinom(x = data_point[[i,"Hos_RSV_1"]],
#                mu = model_point[[i,"RSV_incidence_2"]]*theta[["Rdetect5"]]*theta[["RSV_mult"]],
#                size = theta[["overdispersion"]] # Infinity is a poisson
#                , log=T)
#   
#   return(l + m)
...
# likelihood_inf_points<- function(data_point, model_point, theta, i, log = FALSE)
# {# calculate the likelihood of each point per age group using poisson distributions.
# 
#   l <- dnbinom(x = data_point[[i,"Hos_INF_0"]],
#                mu = model_point[[i,"INF_incidence_1"]]*theta[["Idetect"]] ,
#                size = theta[["overdispersion"]]  # Infinity is a poisson
#                , log=T)
#   
#   m <- dnbinom(x = data_point[[i,"Hos_INF_1"]],
#                mu = model_point[[i,"INF_incidence_2"]]*theta[["Idetect"]]*theta[["detect_multiplier"]],
#                size = theta[["overdispersion"]] # Infinity is a poisson
#                , log=T)
#   
#   # return the sum of the likelihoods for each age group (sum as log scale)
#   return(l+m)
# }
