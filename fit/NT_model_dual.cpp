                  ...
                  double *yout, int *ip)
{
 ...
 ydot[1+i*14]= ( (RSV_Sus[i]*transmission_R[i]*y[0+i*14])
                   - (transmission_I[i]*sigma_both*y[1+i*14])
                   - (gammaR*y[1+i*14])
                   + trickleR[i]
                  );
 ...
 ydot[2+i*14] = ( (gammaR*y[1+i*14])
                    - (rho*y[2+i*14])
                    - (sigma_both*transmission_I[i]*y[2+i*14]) );
  ...
  ydot[4+i*14] = ( (transmission_I[i]*y[0+i*14])
                     - (RSV_Sus[i]*transmission_R[i]*sigma_both*y[4+i*14])
                     - (gammaI *y[4+i*14])
                     + trickleI[i]
  )
    ;
  ...
  //II
  ydot[5+i*14] = ( (transmission_I[i]*y[1+i*14]*sigma_both)
                     + (RSV_Sus[i]*transmission_R[i]*sigma_both*y[4+i*14])
                     - (gammaR*y[5+i*14])
                     - (gammaI*y[5+i*14]) ) ;
                     //
  //P/RI
  ydot[6+i*14] = ( (transmission_I[i]*(sigma_both*y[2+i*14]+y[3+i*14]) )
                     + (gammaR*y[5+i*14])
                     - (gammaI*y[6+i*14]));
  ...
  ydot[7+i*14] = ( (gammaI*y[4+i*14])
                     - (rho*y[7+i*14])
                     - (RSV_Sus[i]*sigma_both*transmission_R[i]*y[7+i*14]) );
                     
                     
                     //IP/R
  ydot[8+i*14] = ((RSV_Sus[i]*transmission_R[i]*(sigma_both*y[7+i*14]+y[9+i*14]))
                    + (gammaI*y[5+i*14])
                    - (gammaR*y[8+i*14]));
  ...
  ydot[12+i*14] =( (transmission_I[i]*y[3+i*14])
                     + (transmission_I[i]*sigma_both*y[2+i*14])
                     + (transmission_I[i]*y[0+i*14]))
                     + trickleI[i]
                     ;
  // Dual cases
  ydot[13+i*14] = ( (transmission_I[i]*y[1+i*14]*sigma_both)
                      + (RSV_Sus[i]*transmission_R[i]*sigma_both*y[4+i*14])
