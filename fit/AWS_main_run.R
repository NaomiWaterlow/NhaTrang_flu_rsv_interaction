...
input_params_theta <- read.csv(here::here("fit","input_params_table_transform.csv"))
input_params_proposal <- read.csv(here::here("fit","input_proposal_table_transform.csv"))
theta_bounds <- read.csv(here::here("fit","theta_bounds.csv"),row.names = 1)
...
colnames(covmat) = rownames(covmat) = names(init_theta)
 ...
 covmat <- as.matrix(nearPD(readRDS(here::here("fit","20210708_dualmult.RDS")))[[1]])/10
...
time_start <- Sys.time()
name <- "Pop_standard"
total_trace <- trace_wrapper(length_run =10,
                             each_run = 5,
                             n_chains = n_chains,
