...
mcmcmc_fn <- function(init_theta, data_to_fit, pop_size, MaxTime = 1e3,
  ...
  chains <- lapply(1:n_chains, function(i)  list( accepted = matrix(NA, nrow  = MaxTime,
                                                   ncol = (4 + n_pars)), 
                   proposed =  matrix(NA, nrow  = MaxTime,
                                      ncol = (4 + n_pars))))
  # set the first row of each trace to 0
  for (i in n_chains){ chains[[i]][["accepted"]][,n_pars + 3] <- 0 }
  ...
  for(s in 1:(MaxTime / indep)){
    ...
    out <-parLapply(cl, 1:n_chains,
                   function(i){
                     ...
                     out <- list( accepted = matrix(NA, ncol = length(theta_names) + 4,
                                   nrow = indep), 
                                  proposed = matrix(NA, ncol = length(theta_names) + 4,
                                                     nrow = indep))
...
# 
                       ...
                       #update the trace
                       out[["accepted"]][t,] <- c(Liklihood_stored, unlist(pars[[i]][theta_names]),
                                    Accepted, 0,0)
                       if(is.finite(Prior_proposed)){
                       out[["proposed"]][t,] <- c(season_out_proposed + Prior_proposed, unlist(proposed[theta_names]),
                                                  Accepted, 0,0)
                       } else {
                         out[["proposed"]][t,] <- c( Prior_proposed, unlist(proposed[theta_names]),
                                                                               Accepted, 0,0)
                                                  }
                     }
                     # print(paste0("t is ", t))
    ...
    for(i in 1:n_chains){
      ...
      chains[[i]][["accepted"]][Interval[,s],] <- out[[i]][["accepted"]]
      chains[[i]][["proposed"]][Interval[,s],] <- out[[i]][["proposed"]]

      # copy parameters from last step of indep trace
      pars[[i]] <- as.list(out[[i]][["accepted"]][indep,][-c(1,n_pars+2, n_pars+3, n_pars+4)])
      names(pars[[i]]) <- theta_names
      # store the likelihood value for each so don't have to recaluclate later

      if(n_chains ==1){
        likelihood <- out[[i]][["accepted"]][indep,][1]
      } else{
        likelihood[[i]]<- out[[i]][["accepted"]][indep,][1]
      }
    }
    ...
    if(n_chains > 1){
      for (swap_test in 1:n_chains){
        ...
        chains[[i]][["accepted"]][Interval[dim(Interval)[1],s], n_pars + 4] <- 1
        ...
        if(R > runif(1)){
          ...
          chains[[i]][["accepted"]][Interval[dim(Interval)[1],s],n_pars+3]<-j
    ...
    if(Interval[1,s] > adapt_start){
      ...
      swap_rates <- sapply(1:n_chains, function(i)
        swapping_rates(chains[[i]][["accepted"]], n_pars = n_pars,
  ...
  swaps_proposed <- sapply(1:n_chains, function(i) swapping_props(chains[[i]][["accepted"]], n_pars=n_pars,
                                                                  prev_swap_proposed = prev_swap_proposed[i]))
  swaps_accepted <- sapply(1:n_chains, function(i) swapping_acceps(chains[[i]][["accepted"]], n_pars=n_pars,
                                                                   prev_swap_accepted = prev_swap_accepted[i]))
...
trace_wrapper <- function(length_run ,
  ...
  total_trace <- lapply(1:(n_chains), function(i)  list(accepted = matrix(NA, nrow  = length_run,
                                                          ncol = (4 + length(init_theta))), 
                                                        proposed = matrix(NA, nrow  = length_run,
                                                                          ncol = (4 + length(init_theta)))))
  ...
  for (pt in 1:(length_run/each_run)) {
    ...
    total_trace <- lapply(1:(length(total_trace)), function(x)
      add_on_wrapper(total_trace[[x]], trace[[x]], pt, each_run))

    # update the init_theta to the new one
    end_trace <- lapply(1:n_chains, function (x)
      as.list(tail(na.omit(total_trace[[x]][["accepted"]]),1)[,2:(length(proposal_sd)+1)]))
...
add_on_wrapper <- function(tt, t, pt, each_run){
   acep <- add_on(tt[["accepted"]], t[["accepted"]], pt, each_run)
   prop <- add_on(tt[["proposed"]], t[["proposed"]], pt, each_run)
   return(list(accepted = acep, proposed = prop))
}
